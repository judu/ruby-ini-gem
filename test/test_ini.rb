...
class TestIni < Test::Unit::TestCase

  def setup
    @ini_file = ::Ini.new 'test/data/good.ini'
    @contents = [
      ['section_one', 'one', '1'],
      ['section_one', 'two', '2'],
      ['section_two', 'three', '3'],
  ...
  def test_class_load
    ini_file = ::Ini.load 'test/data/good.ini'
    assert_instance_of ::Ini, ini_file

    # see if we can parse different style comments
    assert_raise(::Ini::Error) {::Ini.load 'test/data/comment.ini'}

    ini_file = ::Ini.load 'test/data/comment.ini', :comment => '#'
    assert_instance_of ::Ini, ini_file

    # see if we can parse mixed style comments
    assert_raise(::Ini::Error) {::Ini.load 'test/data/mixed_comment.ini'}

    ini_file = ::Ini.load 'test/data/mixed_comment.ini', :comment => ';#'
    assert_instance_of ::Ini, ini_file

    # see if we can parse different style param separators
    assert_raise(::Ini::Error) {::Ini.load 'test/data/param.ini'}

    ini_file = ::Ini.load 'test/data/param.ini', :parameter => ':'
    assert_instance_of ::Ini, ini_file

    # make sure we error out on files with bad lines
    assert_raise(::Ini::Error) {::Ini.load 'test/data/bad_1.ini'}
    assert_raise(::Ini::Error) {::Ini.load 'test/data/bad_2.ini'}
  end
  ...
  def test_each
    ...
    ary = []
    ::Ini.new('temp.ini').each {|*args| ary << args}
    assert_equal [], ary
  end
  ...
  def test_each_section
    ...
    ary = []
    ::Ini.new('temp.ini').each_section {|section| ary << section}
    assert_equal [], ary
  end
  ...
  def test_initialize
    # see if we can parse different style comments
    assert_raise(::Ini::Error) {::Ini.new 'test/data/comment.ini'}

    ini_file = ::Ini.new 'test/data/comment.ini', :comment => '#'
    assert_equal true, ini_file.has_section?('section_one')

    # see if we can parse different style param separators
    assert_raise(::Ini::Error) {::Ini.new 'test/data/param.ini'}

    ini_file = ::Ini.new 'test/data/param.ini', :parameter => ':'
    assert_equal true, ini_file.has_section?('section_one')
    assert_equal '1', ini_file['section_one']['one']
    assert_equal '2', ini_file['section_one']['two']

    # make sure we error out on files with bad lines
    assert_raise(::Ini::Error) {::Ini.new 'test/data/bad_1.ini'}
    assert_raise(::Ini::Error) {::Ini.new 'test/data/bad_2.ini'}
  end
